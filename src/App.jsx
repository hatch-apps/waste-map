...
function cascade(sid, nv) {
  ...
  Object.entries(s.sensitivity).forEach(([t, v]) => {
    ...
    if (n) { R[t] = Math.max(0, n.base * (1 + (r - 1) * v)); L1[t] = R[t]; touched.add(t); }
  });
  const L2 = {};
  Object.entries(L1).forEach(([id, val]) => {
    const n = NODES.find(x => x.id === id); if (!n?.sensitivity) return;
    Object.entries(n.sensitivity).forEach(([t, v]) => {
      if (t === sid) return;
  ...
  Object.entries(L2).forEach(([id, val]) => {
    const n = NODES.find(x => x.id === id); if (!n?.sensitivity) return;
    Object.entries(n.sensitivity).forEach(([t, v]) => {
      if (t === sid || L1[t]) return;
      const tn = NODES.find(x => x.id === t); if (!tn) return;
      const d = (val / n.base - 1) * v * 0.3;
